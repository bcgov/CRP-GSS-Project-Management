...
def pmbok_dashboard():
    ...
    def update_dashboard():
        ...
        try:
            ...
            # Run enhanced_get_projects.py to get latest data
            script_path = os.path.join(os.path.dirname(__file__), 'enhanced_get_projects.py')
            if os.path.exists(script_path):
                try:
                    ...
                    if result.returncode == 0:
                        print(f"✅ enhanced_get_projects.py executed successfully")
                        print(f"Output: {result.stdout[-200:]}")  # Show last 200 chars of output
                    else:
                        print(f"Warning: enhanced_get_projects.py failed: {result.stderr}")
                except subprocess.TimeoutExpired:
                    print("Warning: enhanced_get_projects.py timed out")
                except Exception as e:
                    print(f"Warning: Could not run enhanced_get_projects.py: {e}")
            else:
                print(f"Warning: enhanced_get_projects.py not found at {script_path}")
